...
class UIManager {
    constructor() {
        ...
        this.dynamicOverlayIds = ['progressDialog', 'placeholderModal', 'navigationConfirmModal'];
        this.init();
    }
    ...
    syncBodyLockState() {
        const hasVisibleManagedModal = this.managedModalIds.some((modalId) => {
            const modal = document.getElementById(modalId);
            return modal && modal.style.display === 'flex';
        });

        const hasVisibleDynamicOverlay = this.dynamicOverlayIds.some((overlayId) => {
        ...
        const hasVisibleTransientOverlay = Array.from(document.querySelectorAll('[data-ui-overlay="transient"]'))
            .some((overlay) => overlay.style.display !== 'none');

        const hasVisibleModal = hasVisibleManagedModal || hasVisibleDynamicOverlay || hasVisibleTransientOverlay;
    ...
    showModal(modalId, keepOthersOpen = false) {
        ...
        this.syncBodyLockState();
        
        debugLog('Modal shown successfully:', modalId);
    }

    hideAllModals() {
        this.managedModalIds.forEach(modalId => {
            const modal = document.getElementById(modalId);
        ...
        // Dinamik olusan tam ekran katmanlar tiklamayi bloklamasin
        this.dynamicOverlayIds.forEach((overlayId) => {
            const overlay = document.getElementById(overlayId);
            if (overlay) {
    ...
    hideModal(modalId) {
        ...
        // Gecis suresince gorunmez modalin tiklamalari engellemesini onle
        modal.style.pointerEvents = 'none';
    ...
    confirmDialog(message, title = 'Onay') {
        return new Promise((resolve) => {
            ...
            dialog.dataset.uiOverlay = 'transient';
    ...
    showProgress(progress = 0, text = '') {
        ...
        if (!progressDialog) {
            ...
            progressDialog.dataset.uiOverlay = 'transient';
    ...
    clearBlockingOverlays(aggressive = false) {
        this.dynamicOverlayIds.forEach((overlayId) => {
            const overlay = document.getElementById(overlayId);
        ...
        document.querySelectorAll('[data-ui-overlay="transient"]').forEach((overlay) => {
        ...
        if (aggressive) {
            const keepIds = new Set([...this.managedModalIds, ...this.dynamicOverlayIds]);
            const rootOverlays = Array.from(document.body.children).filter((node) => {
